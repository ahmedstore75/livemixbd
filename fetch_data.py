...
HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (Linux; Android 13; SM-G998B) AppleWebKit/537.36 (KHTML,"
...
try:
  ...
  if response.status_code == 200:
    ...
    for ch in raw_list:
      slug = (
          str(ch.get("slug") or ch.get("id") or "")
          .strip()
          .lower()
          .replace(" ", "_")
      )
      ...
      ch_group = (
          ch.get("category_name") or ch.get("category") or "Toffee Live"
      )
      ...
      # স্ক্রিনশট অনুসারে লিঙ্ক ফরম্যাট (playlist.m3u8)
      formatted_m3u8 = f"{BASE_CDN}/{slug}/playlist.m3u8"

      # কুকি ও ইউজার এজেন্ট ফরম্যাট
...
# --- ৪. toffee.m3u তৈরি (স্ক্রিনশটের ফরম্যাট অনুযায়ী) ---
m3u_lines = []
...
print(
    f"Generated {len(all_channels)} channels matching the exact playlist"
    " format."
)
